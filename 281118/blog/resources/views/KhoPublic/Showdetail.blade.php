...
<div class="container-fluid">
	<div class="row">
        <?php 
            $Id['Khotong'] = "Kho tổng";
             $Id['Thuongmai'] = "Thương mại";
             $Id['CSVC'] = "CSVC";
         ?>
		<div class="col-sm-4"><img src="{{URL::asset('layouts/images/QLKho')}}/{{$key->Hinh}}" style="width: 100%"></div>
		<div class="col-sm-8">
	
			<h1 style="text-align: center; color: red;">{{$key->Ten}}  </h1>
		  <ul>	
			<li><p><h2>Mã vật tư:  {{$key->MVT}}  </h2></p></li>
		  <li><p><h2>Số lượng hiện có: {{$count}} (@foreach($countdetail as $key) {{$Id[$key->Bophan]}}:{{$key->Sl}}, @endforeach)</h2></p> </li>
			<li><p><h2>Đơn vị tính: {{$key->DVT}}</h2></p></li>
		<li>	<p><h2>Thông số kĩ thuật:</h2></p></li>
			<p style="font-size: 20px; font-family: Arial;">	{{$key->Thongso}}</p>
	</ul>
	</div></div>
