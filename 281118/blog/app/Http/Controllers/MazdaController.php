...
class MazdaController extends Controller
{
...
// );
  ...
  $eloquentEvent = MazdaModel::first(); //EventModel implements MaddHatter\LaravelFullcalendar\Event
...
; 
        ...
        return view('datxe.mazda', compact('calendar') );
    }
...
public function addEvent(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('/mazda')->withInput()->withErrors($validator);
        }
     	$di = $request->di;
     	$toi = $request->toi;
     	$convertbd =  str_replace('T',' ',$request->start_date.":00");
     	$convertkt =  str_replace('T',' ',$request->end_date.":00");
        $dieukien = \DB::Table('mazda')->Select()->where('start','<=',$convertbd)->where('end','>=',$convertkt)->count();
     ...
     if($dieukien>=1)
       {  \Session::flash('datphong','Xe đã đặt');
        return Redirect::to('/mazda');
    }
      else 
      { $event = new MazdaModel;
