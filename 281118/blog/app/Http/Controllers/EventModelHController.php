...
class EventModelHController extends Controller
{
...
// }
		$events = \DB::table('event_models_phonghop')->get();
    	$event_list = [];
...
; 
        ...
        return view('datlich.phonghop', compact('calendar') );
    }
...
public function addEvent(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('/phong-hop')->withInput()->withErrors($validator);
        }
     	
     	$convertbd =  str_replace('T',' ',$request->start_date.":00");
     	$convertkt =  str_replace('T',' ',$request->end_date.":00");
        $dieukien = \DB::Table('event_models_phonghop')->Select()->where('start','<=',$convertbd)->where('end','>=',$convertkt)->count();
        $phongban = DSNVlogin::Select('Phongban')->Where('Manv',Auth::user()->Manv)->get();
     ...
     if($dieukien>=1)
       {  \Session::flash('datphong','Phòng đã đặt');
        return Redirect::to('/phong-hop');
    }
      else 
      { $event = new EventModelH;
