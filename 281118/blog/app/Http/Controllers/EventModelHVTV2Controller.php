...
use Mail;
class EventModelHVTV2Controller extends Controller
{
...
// }
		$events = \DB::table('event_models_tuvan2_honviet')->get();
    	$event_list = [];
...
; 
        ...
        return view('datlich.honviet.tuvan2', compact('calendar') )->with('email',$users);
    }
...
public function addEvent(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('/phong-tv2-hv')->withInput()->withErrors($validator);
        }
     	
     	$convertbd =  str_replace('T',' ',$request->start_date.":00");
     	$convertkt =  str_replace('T',' ',$request->end_date.":00");
        $dieukien = \DB::Table('event_models_tuvan2_honviet')->Select()->where('start','<=',$convertbd)->where('end','>=',$convertkt)->count();
     ...
     if($dieukien>=1)
       {  \Session::flash('datphong','Phòng đã đặt');
        return Redirect::to('/phong-tv2-hv');
    }
      else 
      { $event = new EventModelHVTV2;
