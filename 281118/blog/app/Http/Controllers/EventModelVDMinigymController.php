...
use Mail;
class EventModelVDMinigymController extends Controller
{
...
// }
		$events = \DB::table('event_models_mini_gym_viendong')->get();
    	$event_list = [];
...
; 
        ...
        return view('datlich.viendong.minigym', compact('calendar') )->with('email',$users);
    }
...
public function addEvent(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('/phong-Minigym-vd')->withInput()->withErrors($validator);
        }
     	
     	$convertbd =  str_replace('T',' ',$request->start_date.":00");
     	$convertkt =  str_replace('T',' ',$request->end_date.":00");
        $dieukien = \DB::Table('event_models_mini_gym_viendong')->Select()->where('start','<=',$convertbd)->where('end','>=',$convertkt)->count();
     ...
     if($dieukien>=1)
       {  \Session::flash('datphong','Phòng đã đặt');
        return Redirect::to('/phong-Minigym-vd');
    }
      else 
      { $event = new EventModelVDMinigym;
