...
use Mail;
class EventModelVDYogaController extends Controller
{
...
// }
		$events = \DB::table('event_models_yoga_viendong')->get();
    	$event_list = [];
...
; 
        ...
        return view('datlich.viendong.yoga', compact('calendar') )->with('email',$users);
    }
...
public function addEvent(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('/phong-Yoga-vd')->withInput()->withErrors($validator);
        }
     	
     	$convertbd =  str_replace('T',' ',$request->start_date.":00");
     	$convertkt =  str_replace('T',' ',$request->end_date.":00");
        $dieukien = \DB::Table('event_models_yoga_viendong')->Select()->where('start','<=',$convertbd)->where('end','>=',$convertkt)->count();
     ...
     if($dieukien>=1)
       {  \Session::flash('datphong','Phòng đã đặt');
        return Redirect::to('/phong-Yoga-vd');
    }
      else 
      { $event = new EventModelVDYoga;
